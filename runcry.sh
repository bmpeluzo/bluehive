...
if [ -z $job_id ]; then
	...
	echo "#!/bin/bash
#SBATCH -p teraeth -t 30-0:00:00 -o ${job_id}.out
#SBATCH -N 1 --ntasks-per-node=${cores} --mem=0 -C Gold6448Y,ib

module load /scratch/mruggie8_lab/software/modulefiles/crystal/b2

job_dir=${SCRATCH}/tests/${SLURM_JOB_ID}
